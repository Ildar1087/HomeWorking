...
# n = 5 # кустов
from random import randint # Функция randint позволяет генерировать случайные целые числа в заданном диапазоне
list_1 = list(randint(1, 5) for i in range(int(input('Введите кол-во кустов: '))))
print(list_1)
a = int(input('Введите № куста: '))
res = 0 # заводим новую переменную, куда будем фиксировать результат
if a == 1:
...
print(res, 'ягод')
"""решешени схоже с тем, что Вы показывали нам на семинарском занятии. Суть сводится к тому, что при вводе количества кустов,
запускается счетчик случайных чисел и заполняет список с количеством ягод, потом согласно условию,  читается количество 
собранных ягод с указанного куста и двух его соседних"""